...
def create_channel(channel_id: str, maxsize: int = 0) -> str:
    ...
    logger.info("    通道创建  channel_id=%s  maxsize=%s", channel_id, maxsize)
    return channel_id
...
async def send_to_channel(channel_id: str, data: Any) -> None:
    ...
    """
    _data_summary = json.dumps(data, ensure_ascii=False, default=str) if isinstance(data, dict) else str(data)
    logger.info("    通道发送  channel_id=%s  data=%s", channel_id, _data_summary[:500] if len(_data_summary) > 500 else _data_summary)
...
async def receive_from_channel(
    ...
    if item is _CLOSED:
        ...
        logger.info("    通道已关闭  channel_id=%s", channel_id)
        raise ChannelClosedError(f"通道 '{channel_id}' 已关闭，无更多数据")

    _item_summary = json.dumps(item, ensure_ascii=False, default=str) if isinstance(item, dict) else str(item)
    logger.info("    通道接收  channel_id=%s  data=%s", channel_id, _item_summary[:500] if len(_item_summary) > 500 else _item_summary)
    return item
...
def close_channel(channel_id: str) -> None:
    ...
    """
    q = _get_queue(channel_id)
    logger.info("    通道关闭  channel_id=%s", channel_id)
    # 先放入哨兵，再删除注册表项
