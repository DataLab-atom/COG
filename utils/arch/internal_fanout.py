...
设计原则（三阶段全量并发）：
  ...
  Phase 3：main.py 最后定义（可引用所有已定义节点，确保评估入口函数签名完整）
...
架构例外说明（LLM 调用）：
  本函数在输入源层直接调用 LLM，违反"输入源层不得调用 LLM"的通用约定。
  原因：三阶段并发定义需要：
    1. 跨任务共享累积状态（all_ids / all_types）——map 步骤无法表达任务间共享可变状态
    2. overload fn 等待 init_fn 完成后的全局 id 快照作为上下文
  这些需求超出了框架 map 步骤 item_depends_on 的表达能力。
  如需修复，须扩展框架支持"跨任务共享状态"的 map 变体。
"""
...
async def arch_internal_fanout(
    ...
    # ── 共享可变状态 ──────────────────────────────────────────────────────────
    files_dict: dict[str, dict] = {f["id"]: copy.deepcopy(f) for f in files if "id" in f}
    all_types: list[dict] = []
    all_fns:   list[dict] = []
    ...
    # main 文件：root=True 且 id 以 /main 结尾
    main_file_ids = {
        fid for fid, f in files_dict.items()
        if f.get("root") and fid.rstrip("/").endswith("/main")
    }

    # ══ Phase 2：并发定义所有非 main 文件的函数 ════════════════════════════════
    non_main_tasks = [
        _define_fn(file_id, fn_id)
        for file_id, f in files_dict.items()
        if file_id not in main_file_ids
        for fn_id in list(f.get("functions", []))
    ]
    await asyncio.gather(*non_main_tasks)

    # ══ Phase 3：最后定义 main.py 中的函数（可引用所有已定义节点）════════════
    main_fn_tasks = [
        _define_fn(file_id, fn_id)
        for file_id in main_file_ids
        for fn_id in list(files_dict[file_id].get("functions", []))
    ]
    await asyncio.gather(*main_fn_tasks)
