...
def arch_resolve_needs(tree: list[dict]) -> ArchResolveNeedsResult:
    ...
    file_map: dict[str, dict] = {n["id"]: n for n in tree if n.get("kind") == "file" and "id" in n}
    fn_map:   dict[str, dict] = {n["id"]: n for n in tree if n.get("kind") == "function" and "id" in n}
    all_ids: set[str] = {n["id"] for n in tree if "id" in n}
