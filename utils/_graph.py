    ...
    while level_idx < len(levels):
        ...
        if len(async_steps) == 1:
            ...
            results = await asyncio.gather(
                *[_run_step(s, ctx_snapshot) for s in async_steps],
                return_exceptions=True,
            )
            for step, result in zip(async_steps, results):
        ...
        for step in level:
            ...
            # 注入 carry 变量到 context
            for key, wire in loop_cfg.get("carry", {}).items():
                context[key] = _resolve_wire(wire, context)

            # 清除从目标层到当前层的步骤输出（重新执行时会覆盖）
            for clear_idx in range(jump_target_idx, level_idx + 1):
                for s in levels[clear_idx]:
                    all_outputs.pop(s["id"], None)
                    context.pop(s["id"], None)
...
async def _step_as_tagged_stream(
    ...
    if step["type"] != "agent":
        raise ValueError(
            ...
            "仅支持 'agent' / 'tool' / 'input' / 'pipeline' / 'graph'"
        )
...
async def _execute_step_for_events(
    ...
    - 非 graph 类型步骤：直接调用 _run_step，返回 ("__result__", output) 哨兵。
    - graph 类型步骤：递归调用 run_graph_events，透传子图所有内部事件，
      最后返回 ("__result__", final_output) 哨兵。
